...
class _QuantitySelectionState extends State<QuantitySelection> {
  ...
  void _onFocusChange() {
    if (_focusNode?.hasFocus ?? false) {
      ...
      var value = int.tryParse(_textController.text) ?? widget.minQuantity ?? 1;
      if (value < (widget.minQuantity ?? 1)) {
        value = widget.minQuantity ?? 1;
      }
  ...
  bool _validateQuantity([int? value]) {
    final minQty = widget.minQuantity ?? 1;
    if ((value ?? currentQuantity) < minQty) {
      return false;
    }
  ...
  void _onChangeValueQuantity(int quantity, {void Function()? onFalse}) {
    final minQty = widget.minQuantity; // Get the minimum quantity
    ...
    if (!isChanged) {
      if (onFalse != null) {
        ...
        setState(() {
          // Ensure the value is not below minQuantity
          final value = int.tryParse(widget.value?.toString() ?? '$minQty') ?? minQty;
          _textController.text = value! >= minQty! ? value.toString() : minQty.toString();
        });
      }
    }
  }
  ...
  void _onChangeQuantityData(int quantity) {
    if (widget.quantityStep != null && widget.quantityStep! > 1) {
      setState(() {
        _textController.text = quantity >= 0 ? quantity.toString() : (widget.minQuantity ?? 1).toString();
      });
      ...
      EasyDebounce.debounce(
        ...
        _onChangeValueQuantity(
          ...
          onFalse: () {
            setState(() {
              final value = int.tryParse(_textController.text) ?? (widget.minQuantity ?? 1);
              final step = value ~/ widget.quantityStep!;
              var valueQuantity = step * widget.quantityStep!;
              valueQuantity = valueQuantity >= (widget.minQuantity ?? 1) ? valueQuantity : (widget.minQuantity ?? 1);
